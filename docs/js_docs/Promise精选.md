...
在 Promise 出现以前，处理大量的异步网络请求，大概是这样：
...
于是，Promise 规范诞生了，并且在业界有很多实现来解决回调地狱的痛点。比如著名的 Q 和 bluebird，bluebird 甚至号称运行最快的类库。
...
比较一下这种写法和上面的回调式的写法，Promise 的写法更为直观，并且能够在外层捕获异步函数的异常信息。
...
上面是 resolve 方法的解释，传入不同类型的 value 值，返回结果也有区别。这个 API 比较重要，建议大家通过练习一些小例子，并且配合上面的解释来熟悉它。如下例子：
...
 为了便于理解 Promise，除了要多加练习以外，最好的方式是能够将Promise的机制与现实生活中的例子联系起来，这样才能真正得到消化。
